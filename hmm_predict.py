...
class HMMRecognition:
    def __init__(self):
        ...
        self.class_names = ['batden', 'batquat','tatden','tatquat', 'dongcua','mocua']
    ...
    def record(self):
        ...
        frames = []
        myrecording = sd.rec(int(RECORD_SECONDS * RATE), dtype='int16', channels=1)
        sd.wait()
        write(self.record_path, RATE, myrecording)
...
if __name__ == '__main__':
    ...
    hmm_reg.predict(file_name='datasets/batden/batden(1).wav')
    hmm_reg.predict(file_name='datasets/tatden/tatden(1).wav')
    hmm_reg.predict(file_name='datasets/batquat/batquat(1).wav')
    hmm_reg.predict(file_name='datasets/tatquat/tatquat(1).wav')
    hmm_reg.predict(file_name='datasets/dongcua/dongcua(1).wav')
    hmm_reg.predict(file_name='datasets/mocua/mocua(1).wav')
    hmm_reg.record()
    hmm_reg.predict()
