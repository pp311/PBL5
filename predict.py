...
model = {}
class_names = ['tat', 'bat', 'mot', 'hai', 'ba', 'sheila', 'den', 'quat']
audio_format = 'wav'
...
def predict(file_name=None):
    ...
    if start_trim >= 800 or start_trim < 100:
        print("XXX")
        return
...
def sd_callback(rec, frames, time, status):
    ...
    if res == 'sheila':
        ...
        myrecording = sd.rec(int(4 * RATE), channels=1, samplerate=RATE, dtype='int16')
        sd.wait()
        print('Sheila stop listening')
        write(record_path, RATE, myrecording)
        rate, data = read(record_path)
        reduced_noise = nr.reduce_noise(y=data, sr=RATE)
        write("temp/5secnoise.wav", RATE, reduced_noise)
...
while True:
    ...
    sd.wait()
    # write(record_path, RATE, myrecording)
    # print(predict(record_path))
    sd_callback(myrecording, None, None, None)
